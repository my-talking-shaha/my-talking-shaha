...
| ACC-02 | Sign in/sign up with Yandex ID | As a user, I want to be able to sign up/sign in with a Yandex account so that the login process is faster and does not require entering additional passwords. | Could |   • The login screen includes a "Sign in with Yandex ID" button
  • Upon clicking, the system delegates authentication to the Yandex OAuth flow
  • After successful Yandex authorization:if no account exists for this Yandex ID, a new account is created;if an account exists, the user is signed in
  • The new account is permanently linked to the Yandex ID
  • The email received from Yandex is stored as the user's primary email
  • On Yandex authorization failure, the system displays a clear, user-friendly error message |
| ACC-03 | Add multiple cars | As a user, I want to add multiple cars to my account with make, model, color, technical characteristics and details (or select a car from the common catalog if it exists there) so that I can manage all cars in one place. | Must |   • The user can open the "Add car" form
  • The form includes the following fields: make, model, year, color, current mileage, engine type
  • The system allows selecting make and model either from a predefined catalog or via manual entry
  • All fields except color are required
  • After saving, the car appears in the "Garage" list
  • The number of cars a user can add is unlimited. |
...
| Short name | User story title | User story description | Priority | Acceptance criteria |
| --- | --- | --- | --- | --- |
| DATA-01 | Upload vehicle history | As a user, I want to be able to upload data about my car (repairs, mileage, technical condition, trips, refueling) so that I can conveniently store the entire history in a single digital space (car timeline). | Must |   • The user can add a repair record with: date, mileage, description, cost, replaced parts, photo (optional)
  ...
  • All records are automatically linked to the selected car.Records are displayed in chronological order
  • The user can edit and delete any record
  • After adding, editing, or deleting a record, the record list updates automatically (no manual refresh required)
  • The system does not allow entering a mileage value lower than any previously recorded mileage for that car |
| DATA-02 | Update parts and condition | As a user, I want to upload data about newly installed parts or select them from a catalog so that I can keep up-to-date information about the car’s condition. | Should |   • The user can add a part with: part name, installation date, mileage at installation, lifespan (km), photo (optional)
  • The user can select a part from a predefined catalog (e.g., oil, filter, timing belt, brake pads)
  • When selected from the catalog, the system may auto-fill a typical lifespan if known
  • The user can manually edit the part's lifespan
  • The user can mark a part as **replaced**
  • When this happens:the current part record is preserved with a "replaced" flag;a new part record is created (with new date and mileage) OR the user is guided to the "add part" flow. |
...
| STATUS-02 | Remaining part lifespan | As a user, I want to see the remaining “lifetime” of parts (predicted lifespan) so that I can plan replacements before a failure occurs. | Must |   • For each part with a defined lifespan, the remaining lifespan is displayed:in kilometers;and/or as a percentage of the initial lifespan
  • Calculation formula: remaining = initial lifespan − (current mileage − mileage at installation)
  • If lifespan is not defined for a part, "Lifespan not set" is shown
  • If remaining lifespan is less than 10% of initial lifespan — the part is highlighted in yellow (or other visual warning)
  • If remaining lifespan is ≤ 0 km — the part is highlighted in red (replacement required)
  • The user can manually adjust the initial lifespan value of any part |
...
| CHAT-03 | Start screen — chat + quick questions | As a user, I want to be able to immediately type in the chat with my car or pick a question from quick questions when I open the app so that I can quickly get the information I need and navigate the app faster. | Must |   • The home screen is the AI chat
  • Below the input field, quick-action buttons are displayed.Starter set includes: "Car status", "When is next service?", "What might break?", "Add refuel", "Log repair"
  • The system may adapt the set of quick questions based on car data (e.g., "Time to change oil?" when oil lifespan remaining &lt; 1000 km)
  • Tapping a button sends the corresponding text as a user message to the chat. |
...
| CHAT-05 | Chat → form | As a user, I want to tell the AI about an event (repair, part replacement, refuel, trip) so that the AI redirects me to a pre-filled form where I can quickly enter all data and save it. | Must |   • The user can write a natural-language message describing an event: repair, part replacement, refuel, or trip
  • The AI recognizes the event type (intent) and extracts relevant attributes (e.g., part name, mileage, date)
  • The system responds with: "Would you like to add [event type]?" and a "Go to form" button
  • Tapping the button opens the corresponding form, pre-filled with extracted data where possible
  • The user can complete or modify any field and save.After saving, the system returns to the chat, and the AI confirms the addition
  • The user may cancel the action at any step before saving. |
